    ...
    try:
        ...
        if pg_conn:
            ...
            if result and result[0] > 0:
                ...
                result = cursor.fetchone()
                if result:
                    metrics['records_processed']['personal_data_individuals'] = result[0]
...
def get_recent_errors():
    ...
    try:
        ...
        errors.append(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - INFO - Monitoring active")
...
async def unregister(websocket):
    """Unregister a client"""
    connected_clients.discard(websocket)  # Use discard instead of remove to avoid KeyError
